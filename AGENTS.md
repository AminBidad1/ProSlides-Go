...
| PostgreSQL | PostgreSQL 16; migrations `0001`-`0016`; authoritative users/content/settings/editor revisions/access codes/OTP and reset hashes/sessions/frozen run definitions/answers/scores/events | Durable data belongs here. Add forward-only migrations only. |
...
| 2026-09-17 | Added durable participant rejoin and away-name restoration | Migration `0016` adds nullable `participants.disconnected_at`; participant SSE open/close clears/sets it. Rejoining the same session with a new credential restores the existing participant row (same id/answers/score, credential and avatar rotated, count unchanged, no presence event); an actively connected duplicate name still returns `409 display_name_taken`; a new run is a fresh session. Host reconnection resolves the same non-ended session idempotently. OpenAPI, `proslides_live_joins_total{outcome="joined|restored"}`, service/store/HTTP tests, and the Compose integration rejoin/host-resume assertions were added. Go fmt/vet/tests incl. `-race` on live, web lint/typecheck/49 unit tests/bundle build/OpenAPI drift check, both Compose configs, and a real-stack rejoin run (SSE-close presence, restore 200 with preserved score, 409 takeover) passed. |
| 2026-09-17 | Anchored live timers to the server deadline instead of client clock | Participant and manager snapshots now carry server-computed `remaining_seconds` (PostgreSQL clock: `GREATEST(0,ROUND(EXTRACT(EPOCH FROM ends_at-clock_timestamp())))::int`, present only while `question_open`), the exact window `SubmitAnswer` admits. `normalizeLiveSlide` prefers it over the old `ends_at - Date.now()` derivation, and the local player fallback re-anchors a reconnecting question from `snapshot.session.remaining_seconds`, so a reconnected player mid-question counts down from the true remaining time rather than restarting (client clock skew no longer inflates or shrinks the countdown). No migration/env/SSE event change. OpenAPI `LiveSession`/`PublicLiveSession`, Go snapshot SQL/tests, `protocol.js`, `PresentationEntry.jsx`, and timer/protocol unit tests (53 total) were updated. Go fmt/vet/tests incl. `-race` on live, web lint/typecheck/53 unit tests/budgeted build/OpenAPI drift check, and both Compose configs passed. |
